...
variable "parameter_tier" {
  type        = string
  default     = "bronze"
  description = "Parameter tier applied to the storage account (replication, networking posture, defaults). Valid options: bronze, silver, gold, platinum, diamond, custom."
  validation {
    condition = (
      contains(["bronze", "silver", "gold", "platinum", "diamond", "custom"], var.parameter_tier) &&
      (var.parameter_tier != "custom" || var.parameter_tier_custom != null)
    )
    error_message = "parameter_tier must be bronze, silver, gold, platinum, diamond, or custom. Provide parameter_tier_custom when using custom."
  }
}
...
variable "parameter_tier_custom" {
  type = object({
    ...
    allowed_public_ip_addresses       = optional(list(string), [])
  })
  default     = null
...
variable "allowed_public_ip_addresses" {
  type        = list(string)
  default     = []
  description = "List of public IP addresses or IP ranges in CIDR format to allow access from. Only IPv4 addresses are allowed. Used when public_network_access_enabled is true and network_rules_default_action is Deny."
}
