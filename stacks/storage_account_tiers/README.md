...
Each capability offers curated Bronze/Silver/Gold (and, for parameter tiers, Platinum/Diamond) defaults plus a `custom` escape hatch to override any attribute while reusing the stack’s wiring for networking, RBAC, and alerts.
...
| **Gold** | Mission-critical | **GZRS** | **30d / 30d** | Deny-by-default; **Private Endpoint recommended** | Region-paired writes with zone redundancy in primary; adds cross-region durability | `account_replication_type = "GZRS"` • `enable_geo_priority_replication = false` |
| **Platinum** | Tier-1 workloads | **GZRS** | **30d / 30d** | Deny-by-default; **Private Endpoint only** | Same as Gold plus geo-priority replication for blobs via AzAPI | `account_replication_type = "GZRS"` • `enable_geo_priority_replication = true` |
| **Diamond** | Regulated / DR-ready | **RAGZRS** | **30d / 30d** | Deny-by-default; **Private Endpoint only** | Read-access geo + zone redundancy, blob geo priority enabled | `account_replication_type = "RAGZRS"` • `enable_geo_priority_replication = true` |
...
- The stack now requires the `azapi` provider (same subscription as infra by default) so Platinum/Diamond parameter tiers can toggle Geo Priority Replication via AzAPI.
